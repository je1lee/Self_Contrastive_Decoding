...
class OPTForCausalLM(OPTPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.student_lm_head = None
        self.input_ids_student = None
        # Initialize weights and apply final processing
        self.post_init()
    ...
    def forward(
        ...
        last_hidden_reshape = outputs[0].view(1,-1) 
        stacked_hidden_states = torch.stack(outputs[2], dim=0).view(len(outputs[2]), -1)

        # print(contrastive_index)
        if self.outside:
            if input_ids.shape[1] != 1:
                self.input_ids_student = input_ids.clone().detach()
            else:
                self.input_ids_student = torch.cat((self.input_ids_student, input_ids), dim=-1)
            
            student_outputs = self.student_model(
                input_ids = self.input_ids_student,
            )
            student_logits = student_outputs["logits"][:,-1,:]

        else:
            cos_sim = torch.nn.functional.cosine_similarity(last_hidden_reshape, stacked_hidden_states, dim=1)
            for i in range(cos_sim.shape[0]):
                ...
                if cos_sim[i] >= 0.75:
                    ...
                    contrastive_index=i
            student_logits = self.lm_head(outputs[2][contrastive_index]).contiguous()
        
        if self.contrastive: 
            logits = contrastive_calculation(logits, student_logits)
        else:
            pass
...
def contrastive_calculation(upper_logit_input, lower_logit_input, scaling_factor=0.1):
    calc_logits_upper = upper_logit_input.softmax(dim=-1)
    calc_logits_lower = lower_logit_input.softmax(dim=-1)
    top_prob_threshold = calc_logits_upper.max(-1).values.squeeze(-1) * torch.tensor(scaling_factor)
    device = calc_logits_upper.device
    neg_infinity = torch.tensor(-torch.inf).to(device)
    threshold_expanded = expand_tensor(top_prob_threshold, calc_logits_upper.shape)
    modified_logits = torch.where(calc_logits_upper >= threshold_expanded, torch.log(calc_logits_upper) - torch.log(calc_logits_lower), neg_infinity)
